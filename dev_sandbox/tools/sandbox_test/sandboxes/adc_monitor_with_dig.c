...
//	memset( &meleeMenuFont, 0, sizeof( meleeMenuFont ) );
	...
	REG_WRITE( IO_MUX_GPIO7_REG, 3<<FUN_DRV_S | 2<<MCU_SEL_S | 	0<<FUN_PU_S );

	// For mic
	REG_WRITE( GPIO_ENABLE_W1TC_REG, 1<<(8) );
	REG_WRITE( GPIO_OUT_W1TC_REG, 1<<(8) );
	REG_WRITE( GPIO_PIN8_REG, 0 );
	REG_WRITE( IO_MUX_GPIO8_REG, 0<<FUN_DRV_S | 2<<MCU_SEL_S  | 0<<FUN_PU_S );

	DPORT_REG_WRITE( GPIO_FUNC8_IN_SEL_CFG_REG, PRO_ALONEGPIO_IN0_IDX ); 
	DPORT_REG_WRITE( GPIO_FUNC8_IN_SEL_CFG_REG, PRO_ALONEGPIO_OUT0_IDX ); 
	...
	// This disables the filters.  They don't work the way you think.
	// they are per DIG.  So, if you are polling any more than 1 pin
	// the filters don't work.
	WRITE_PERI_REG( APB_SARADC_FILTER_CTRL_REG,
		 0  // Disable filters	
		//3<<30 |
		//30<<16 |
		//30<<23 
	...
	const int sar_clk_div = 4*1; // default is 4. (0 is invalid)
	...
	WRITE_PERI_REG(APB_SARADC_SAR1_PATT_TAB1_REG,0x53736373);	// set adc1 channel & bitwidth & atten  
...
void sandbox_tick()
{
	...
	for( x = 0; x < 240; x++ )
	{
		...
		int color = 0;
		int ictx = 0;
		for( ictx = 0; ictx < 4; ictx++ )
		{
			uint16_t tp = safe_buf[i++];
			if( ( tp & 0xf800 ) == 0x3800 ) { color = 16; hasall |= 1; }
			else if( ( tp & 0xf800 ) == 0x0000 ) { color = 30; hasall |= 2; }
			else if( ( tp & 0xf800 ) == 0x2800 ) { color = 160; hasall |= 4; }
			else if( ( tp & 0xf800 ) == 0x3000 ) { color = 180; hasall |= 8; }
			else continue;
