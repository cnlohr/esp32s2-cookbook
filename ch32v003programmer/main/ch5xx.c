/*
	...
	Copyright 2025-2026 monte-monte
*/
...
unsigned char ch5xx_write_block_bin[236] = {
	...
	if (clock == 0) {
		...
		case CHIP_CH570:
			...
			if ((rr&0xff) != 0x48) {
				...
				ch5xx_write_safe(dev, 0x40001805, 0x10, 0); // Flash SCK - this is a value from the startup code
				// Set core clock
...
int ch5xx_read_uuid(struct SWIOState * iss, uint8_t * buffer) {
	...
	int r = MCFReadWord(dev, addr, (uint32_t*)(buffer));
	if (r) {
		return r;
	}
	r = MCFReadWord(dev, addr+4, (uint32_t*)(buffer+4));
	if (r) {
		return r;
	}
