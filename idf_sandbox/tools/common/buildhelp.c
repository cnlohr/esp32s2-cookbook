...
int main( int argc, char ** argv )
{
	...
	do
	{
...

		{
			...
			snprintf( temp, sizeof( temp ) - 1, "-I%s/components/esp_driver_gpio/include", idf_path ); appendcflag( temp );
		...
		FILE * fc = fopen( "build/compilecount.txt", "r" );
		int cc = 0;
		if( fc )
		{
			fscanf( fc, "%d", &cc );
		...
		cc++;
		fc = fopen( "build/compilecount.txt", "w" );
		fprintf( fc, "%d\n", cc );
		fclose( fc );
